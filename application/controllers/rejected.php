    ...
    public function index()
    {
        if ($this->session->userdata("primary_key")) {
            if ($this->session->userdata("user_type") == "super admin" || $this->session->userdata("user_type") == "admin") {
                $this->session->set_userdata("title", "Rejected Applications");
                
                if ($this->input->get("category") == md5("Educational"))
                {
                    $this->session->set_userdata("current_tab", "rejected_educational");
                ...
                {
                    $this->session->set_userdata("current_tab", "rejected_scholarship");

                    $data["category"] = "Scholarship";
                }

                $this->load->view('templates/header.php');
                $this->load->view('pages/rejected_view.php', $data);
